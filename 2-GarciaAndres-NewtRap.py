...
encontrar:
    -punto_medio: punto en el que se inicia la iteracion
    -funcion: funcion que va a ser derivada con su derivada se generara la formula de aproximacion
    -intervalo: lsita que contiene los limites del intervalo donde se encuentra la raiz
    -Taylor: expresion generada por x- f(x)/f'(x)
    -Error: espacio entre las iteraciones usado para evaluar si se esta convergiendo
...
def ingresar_precision_solucion():
    precision = raw_input("¿Para cuántos decimales de precision quieres se calcule tus soluciones?\n")
...
def encontrar(intervalos_solucion, precision, funcion):
    ...
    """esta funcion genera la expansión de taylor e itera hasta encontrar una solucion con la 
    precision necesaria"""             
    def encontrar_solucion(intervalo, precision):
        
        punto_medio = (intervalo[1]+intervalo[0])/2 
        taylor = x - ((funcion)/funcion.diff(x))
        x_n = round(punto_medio, precision) 
        x_nmas1 = round(taylor.subs(x, x_n), precision)
        error= x_nmas1 - x_n
        while abs(error) > (10**(-precision)):
            x_n = x_nmas1
            x_nmas1 = round(taylor.subs(x, x_nmas1), precision)
            error = x_nmas1 - x_n
